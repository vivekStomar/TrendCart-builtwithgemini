...
RESOURCE = os.environ.get(
    ...
    "projects/933430779881/locations/us-east4/reasoningEngines/1899890122099261440",
)
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Fallback: pull parts from task history or artifacts.
        if not parts and last_task is not None:
            for msg in getattr(last_task, "history", []):
                role = getattr(msg, "role", None)
                if role == Role.agent or str(role).endswith("agent"):
                    parts.extend(_extract_parts(getattr(msg, "parts", [])))
            if not parts:
